...
class script {
    function comment($category,$plugin,$action=false){
        if (!$action) {
            $reader = new Reader($category, $plugin);
        } else {
            $reader = new Reader($category, $plugin, $action);
        }
...
class Reader
{
	...
	public function __construct()
	{
        ...
        include_once "plugins/".$arguments[1]."/".$arguments[0].".php";
		if($count === 1) {
			throw new \Exception("No zero argument constructor allowed");
		} else if($count === 2) {
			$reflection = new \ReflectionClass($arguments[1]."\\".$arguments[0]);
		} else {
            if ( method_exists($arguments[1]."\\".$arguments[0], $arguments[2] ) ) {
                $reflection = new \ReflectionMethod($arguments[1]."\\".$arguments[0], $arguments[2]);
            }
		}
        if (isset($reflection)) {
